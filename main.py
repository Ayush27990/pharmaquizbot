...
# ======================
# FLASK HEALTH SERVER
# Required by Railway to keep the service alive
# ======================
...
async def scheduled_job(bot):
    try:
        ...
        if score >= 7:
            await send_for_approval(bot, mcq, "Auto: " + topic)
        else:
            logger.info("Low score, regenerating...")
            topic2 = await generate_topic()
            mcq2 = await generate_mcq(topic2)
            if mcq2:
                await send_for_approval(bot, mcq2, "Auto retry: " + topic2)
    except Exception as e:
...
async def post_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await scheduled_job(context.bot)
...
def main():
    ...
    # Start Flask in a background thread for Railway health checks
    ...
    # Use APScheduler instead of PTB job_queue (more reliable on Railway)
    ...
    async def post_init(application):
        scheduler.start()
        logger.info(f"Scheduler started. Interval: {INTERVAL // 60} minutes")
